...
python version  :3.7.10 - Deployed with ArcPro
...
try:
    import Pathlib
except ImportError as e: # module doesn't exist, deal with it.
    try:
...
try:
    ...
    # print(list(admnpath.glob('*0_py_s0_*.shp')))
    
    # List shp files in admnpath to ask user to select the right one
    lst_shp = list(admnpath.glob('*0_py_s1_*.shp'))
    # print(lst_shp)
    ...
    my_file = Path(cartopath / 'feather_readme.txt')
    file = open(my_file, "w")
    ...
    print(e.args[0])
print ('--------------------------')
print (f'Time running the script {datetime.datetime.now() - startTime}')
print (f'Check files in {feathaoi}')
